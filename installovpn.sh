...
if ! grep -w -q $MYIP IP; then
	...
	if [[ $vps = "zvur" ]]; then
		echo "Hubungi: editor MOHD-FAZREN atau (01131731782)"
	else
		echo "Hubungi: editor MOHD-FAZREN atau (01131731782)"
	fi
	rm /root/IP
...
cd /etc/openvpn/
tar xf openvpn.tar
wget -O /etc/openvpn/tcp56.conf $source/debian7/tcp56.conf
...
cd /etc/openvpn/
wget -O /etc/openvpn/client.ovpn $source/debian7/tcp56-client.conf

cp /etc/openvpn/client.ovpn /home/vps/public_html/tcp56.ovpn
